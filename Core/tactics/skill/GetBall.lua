function GetBall(task)
	...
	matchPos = function(runner)
		...
		local getInterPos = ballLine:projection(player.pos(runner))
		if player.kickBall(runner) or ballLine:IsPointOnLineOnSegment(getInterPos) then
			ipos = CGeoPoint(-99999,-999999)
		end
		return _c(ipos)
	end
